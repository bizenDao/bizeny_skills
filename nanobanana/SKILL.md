...
画像生成は `https://bizeny.bon-soleil.com/labo/image_gen/api/generate` を使う。
...
curl -s -X POST https://bizeny.bon-soleil.com/labo/image_gen/api/generate \
  -H "X-API-Key: ${API_KEY}" \
...
- 生成物の保存先: `~/workspace/data/generated/`
- 採用作品は手動で適切な場所にコピー（`~/assets/images/` など）
